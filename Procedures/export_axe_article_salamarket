...
BEGIN
 ...
 unload       
        ...
        cast( omc_f_decimal_to_string_new(sref_article.sra_qte_uni_ref_vente_etiq,3)  + ' ' + unite_etiq.uni_code as varchar(8)) + coalesce(space( 8 - length(omc_f_decimal_to_string_new(sref_article.sra_qte_uni_ref_vente_etiq,3) + ' ' + unite_etiq.uni_code)),space(8)) as CONTENANCE_UNITE,               
        space(7),
        left( cast(article.art_desig_courte as varchar), 30 ) + coalesce(space( 30 - length(cast (article.art_desig_courte as varchar))),space(30)) as LIBELLE_LONG,         
        ...
        left( cast(from_origines.Origines  as varchar ), 15 ) + coalesce(space( 15 - length(cast (from_origines.Origines  as varchar))),space(15)) as ORIGINES,                      
        left( cast(classe_produit.clapro_libelle  as varchar ), 15 ) + coalesce(space( 15 - length(cast (classe_produit.clapro_libelle  as varchar))),space(15)) as CATEGORIE,
        left( cast(from_calibres.Calibres  as varchar ), 15 ) + coalesce(space( 15 - length(cast (from_calibres.Calibres  as varchar))),space(15)) as CALIBRES,
        left( cast(from_traitement.traitement  as varchar ), 15 ) + coalesce(space( 15 - length(cast (from_traitement.traitement  as varchar))),space(15)) as Traitement,
        left( cast(from_Varietes.Varietes  as varchar ), 15 ) + coalesce(space( 15 - length(cast (from_Varietes.Varietes  as varchar))),space(15)) as Varietes,
        space(50) 
    From    
